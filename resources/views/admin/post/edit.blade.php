              ...
              @csrf
                ...
                @if(auth()->user()->likedPosts->contains($post->id) != 1)
                  ...
                  <button type="submit" class="border-0 bg-transparent">
                    <i class="text-warning" style="font-size: large;"><u><strong><a class="text-warning">Поменять статус заявки</a></strong></u></i>
                  </button>
                    @else
                  <button type="submit" class="border-0 bg-transparent">
                    <i><u class="text-danger">Отклонить</u></i> 
                  </button>
                  @endif
                @else
                <p class="text-muted">статус заявки: <strong class="text-success">СОГЛАСОВАНО</strong></p>
                @endif
                @endauth
            </form>
          </div>
          <div class="form-group w-100"></i>
              ...
              <button type="submit" class="border-0 bg-transparent">
                ...
                @if((auth()->user()->dislikedPosts->contains($post->id) != 1))
                  @if(auth()->user()->likedPosts->contains($post->id))
                    <i class="text-warning" style="font-size: large;"><u><strong><a class="text-warning">Поменять статус заявки</a></strong></u></i>
                  @else
                    <i><u class="text-success">Согласовать</u></i> 
                  @endif
                @endif
                @endauth
              </button>
            @else
                <p class="text-muted">статус заявки: <strong class="text-danger">ОТКЛОНЕНО</strong></p>
            @endif
            </form>
          </div>
