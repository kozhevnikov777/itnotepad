        ...
        <div class="row">
            ...
            <form action="{{ route('personal.post.store') }}" method="POST" enctype="multipart/form-data"> 
                ...
                <div class="form-group w-25">
                    <input type="text" class="form-control text-info" name="title" placeholder="Ваше ФИО" value="{{ auth()->user()->name }}">
                    @error('title')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="form-group w-75">
                    <label>Напишите причину</label>
                ...
                <div class="form-group w-50">
                  ...
                  <p class="mb-0 text-muted">Заполнять по типу:&nbsp; </p>
                  <p class="mb-0 text-muted">год-месяц-день &nbsp;часы:минуты</p>
                  <input type="text" class="form-control w-50" name="end_time" placeholder="Укажите время окончания" value="{{ $current }}">
                  @error('end_time')
                    <div class="text-danger">{{ $message }}</div>
                  @enderror
                </div>
                <div class="form-group w-25">
                    ...
                    <p class="mb-0 text-muted">Добавлять по необходимости</p> 
